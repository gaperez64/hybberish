...
"""An over-approximation of the true flow. Generated by TM integration.
"""
struct OverApprox
...
function tm_integration(vector_field_constructor::Function,
                        domain::IntervalBox, k::Integer, J, time_horizon::Float64,
    ...
    # TODO: Currently we're just shifting the ODEs only initially.
    #       Properly implement preconditioning!
    # Counterbalance the shift in ODEs by shifting the domain
    # by the same amount in the opposite direction.
    domain = domain - IntervalBox(shift_offsets)
    ...
    while remaining_time > 0.0
        ...
        if RECOMPUTE_VECTOR_FIELD
            vector_field_tms, shift_offsets =
            vector_field_tmv_shifted(vector_field_constructor, get_variables(), dom_i,
            shift=PRECONDITIONING, fixpoint_callback=fixpoint_callback)
            # FIXME: Do we also want to apply shifting / preconditioning here?
            domain = domain - IntervalBox(shift_offsets)
        end
